...
namespace Libplanet.Tests.Crypto
{
    public class PublicKeyTest
    {
        ...
        private readonly byte[] _key1 = new byte[]
        {
        ...
        public void Verify()
        {
            var pubKey = new PublicKey(_key1);
            ...
            Assert.True(pubKey.Verify(_payload, signature));
            Assert.False(pubKey.Verify(_payload, ImmutableArray<byte>.Empty));
            Assert.False(pubKey.Verify(_payload, default(ImmutableArray<byte>)));
        }
        ...
        public void IPublicKeyVerify()
        {
            ...
            Assert.Equal(
                pub.Verify(_payload, signature),
                publicKey.Verify(_payload, signature));
            Assert.True(pub.Verify(_payload, signature));
            Assert.False(pub.Verify(_payload, ImmutableArray<byte>.Empty));
            Assert.False(pub.Verify(_payload, default(ImmutableArray<byte>)));
            Assert.False(pub.Verify(_payload, signature.Skip(1).ToArray()));
            Assert.False(pub.Verify(_payload, signature.Concat(new byte[] { 0x00 }).ToArray()));
        }
