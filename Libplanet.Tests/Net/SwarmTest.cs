        ...
        public async Task BreadcrumbTrail()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < swarmCount; i++)
                {
                    ...
                    swarms[i] = CreateSwarm(
                        blockChains[i],
                        keys[i],
                        0,
                        host: IPAddress.Loopback.ToString());
                    tasks.Add(StartAsync(swarms[i]));
                }
                ...
                Message reply =
                    await swarms[0].Transport.SendMessageWithReplyAsync(
                        swarms[1].AsPeer as BoundPeer,
                        new Ping(),
                        TimeSpan.FromMilliseconds(Timeout),
                        CancellationToken.None);

                Assert.Equal(4, reply.Trail.Trails.Length);
                string sentTrail =
                    Libplanet.Net.Messages.BreadcrumbTrail.TrailType.Sent.ToString("G");
                string receivedTrail =
                    Libplanet.Net.Messages.BreadcrumbTrail.TrailType.Received.ToString("G");
                string repliedTrail =
                    Libplanet.Net.Messages.BreadcrumbTrail.TrailType.Replied.ToString("G");
                string expected =
                    $"[{sentTrail}:{swarms[0].Address}:{reply.Trail.Trails[0].Timestamp}]/" +
                    $"[{receivedTrail}:{swarms[1].Address}:{reply.Trail.Trails[1].Timestamp}]/" +
                    $"[{repliedTrail}:{swarms[1].Address}:{reply.Trail.Trails[2].Timestamp}]/" +
                    $"[{receivedTrail}:{swarms[0].Address}:{reply.Trail.Trails[3].Timestamp}]";
                Assert.Equal(expected, reply.Trail.ToString());
