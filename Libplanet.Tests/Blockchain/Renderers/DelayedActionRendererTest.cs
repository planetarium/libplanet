...
namespace Libplanet.Tests.Blockchain.Renderers
{
    public class DelayedActionRendererTest : DelayedRendererTest
    {
        ...
        public async Task ClearRenderBufferWhenItsInterval()
        {
            ...
            {
                await chain.MineBlock(key.PublicKey);
                await fork1.MineBlock(key.PublicKey);
                await fork2.MineBlock(key.PublicKey);
                await fork3.MineBlock(key.PublicKey);
            }
        ...
        public async Task DelayedRendererInReorg()
        {
            ...
            var block = await forked.MineBlock(key.PublicKey, append: false);
            forked.Append(
                    block,
            ...
            block = await forked.MineBlock(key.PublicKey, append: false);
            forked.Append(
                    block,
                    evaluateActions: true,
                    renderBlocks: false,
