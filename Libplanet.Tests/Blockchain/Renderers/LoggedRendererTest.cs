    ...
    public class LoggedRendererTest : IDisposable
    {
        ...
        public void ActionRenderings(bool unrender, bool error, bool rehearsal, bool exception)
        {
            ...
            if (error)
            {
                ...
                renderer = new AnonymousRenderer<DumbAction>
                {
            ...
            renderer = new LoggedRenderer<DumbAction>(renderer, _logger, LogEventLevel.Information);
            Assert.False(called);
            ...
            try
            {
                if (error)
                {
                    if (unrender)
                    {
                        renderer.UnrenderActionError(_action, actionContext, actionError);
                    }
                    else
                    {
                        renderer.RenderActionError(_action, actionContext, actionError);
                    }
                }
                else
                {
                    if (unrender)
                    {
                        renderer.UnrenderAction(_action, actionContext, _stateDelta);
                    }
                    else
                    {
                        renderer.RenderAction(_action, actionContext, _stateDelta);
                    }
                }
            }
