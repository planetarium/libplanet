...
{
    private INode? RemoveFromRoot(in PathCursor cursor) => Node switch
    {
        null => null,
        _ => Remove(Node, cursor),
    };

    private INode? Remove(INode node, in PathCursor cursor) => node switch
    {
    ...
    private INode RemoveFromFullNode(FullNode fullNode, PathCursor cursor)
    {
        ...
        {
            return ReduceFullNode(fullNode.SetChild(FullNode.ChildrenCount - 1, null));
        }
    }
