...
{
    private INode Insert(INode? node, in PathCursor cursor, ValueNode value, bool allowNull)
        => node switch
        {
            HashNode hashNode => InsertToHashNode(hashNode, cursor, value, allowNull),
            null => allowNull
    ...
    private INode InsertToHashNode(
        HashNode hashNode, PathCursor cursor, ValueNode value, bool allowNull)
    {
        var unhashedNode = UnhashNode(hashNode);
