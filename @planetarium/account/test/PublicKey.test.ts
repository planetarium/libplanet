...
describe("PublicKey", () => {
  ...
  describe("verify()", () => {
    test("signature w/o high S", async () => {
      ...
      await expect(() =>
        pubKey.verify(invalidMsgType, sig),
      ).rejects.toThrowError(/got string/);

      const invalidSigType = "" as unknown as Signature;
      await expect(() =>
        pubKey.verify(msg, invalidSigType),
      ).rejects.toThrowError(/got string/);
    });

    test("signature with high S", async () => {
      const msg = new Uint8Array([0x01, 0x02, 0x03]);
      const sig = Signature.fromHex(
        "3045022033876cdda6323ecf479198989623660324d68a0398bd9e295999b011de728b5e022100a84eb8ba3b787e29aba026ab50f624c5843c0a65b22c1b73aecb6929e7c87de2",
      );
      const pubKey = PublicKey.fromHex(
        "02c0ece6c66e203b08bd82b1edf1ad67c194e8995745c76f2fe7b36bb9b550b93b",
        "compressed",
      );
      expect(await pubKey.verify(msg, sig)).toBeTruthy();
    });
  });
