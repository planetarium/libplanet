...
namespace Libplanet.Net
{
        ...
        where TBlockAction : IAction, new()
    {
        ...
        public void BroadcastBlocks(IEnumerable<Block<TTxAction, TBlockAction>> blocks)
        {
        ...
        internal IAsyncEnumerable<Block<TTxAction, TBlockAction>> GetBlocksAsync(
            Peer peer,
            IEnumerable<HashDigest<SHA256>> blockHashes)
        {
            ...
            return new AsyncEnumerable<Block<TTxAction, TBlockAction>>(async yield =>
            {
                        ...
                        if (parsedMessage is Messages.Blocks blockMessage)
                        {
                            foreach (byte[] payload in blockMessage.Payloads)
                            {
                                Block<TTxAction, TBlockAction> block =
                                    Block<TTxAction, TBlockAction>.FromBencodex(payload);
...
#pragma warning restore CS4014
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            IAsyncEnumerable<Block<TTxAction, TBlockAction>> fetched = GetBlocksAsync(
                peer,
                message.Hashes
            );

            List<Block<TTxAction, TBlockAction>> blocks = await fetched.ToListAsync(
                cancellationToken
            );
            ...
            CancellationToken cancellationToken)
        {
            ...
            // hashes.
            Block<TTxAction, TBlockAction> tip = _blockChain.Tip;
        ...
        private async Task AppendBlocksAsync(
            Peer peer,
            List<Block<TTxAction, TBlockAction>> blocks,
            CancellationToken cancellationToken
        )
        {
            // We assume that the blocks are sorted in order.
            Block<TTxAction, TBlockAction> oldest = blocks.First();
            Block<TTxAction, TBlockAction> latest = blocks.Last();
            Block<TTxAction, TBlockAction> tip = _blockChain.Tip;
        ...
        private void TransferBlocks(GetBlocks getData)
        {
            ...
            {
                if (_blockChain.Blocks.TryGetValue(hash, out Block<TTxAction, TBlockAction> block))
                {
