...
namespace NetMQ.Core
{
    ...
    internal class Reaper : ZObject, IPollEvents
    {
        ...
        {
            Console.WriteLine($"{{0:dd.MM.yyyy hh:mm.ss:ffff}}[{this}][{System.Threading.Thread.CurrentThread.ManagedThreadId}]: ProcessStop()", DateTimeOffset.UtcNow);
            m_terminating = true;
        ...
        {
            Console.WriteLine($"{{0:dd.MM.yyyy hh:mm.ss:ffff}}[{this}][{System.Threading.Thread.CurrentThread.ManagedThreadId}]: ProcessForceStop()", DateTimeOffset.UtcNow);
            m_terminating = true;
            SendDone();
            m_poller.RemoveHandle(m_mailboxHandle);
            m_poller.Stop();
            Console.WriteLine($"{{0:dd.MM.yyyy hh:mm.ss:ffff}}[{this}][{System.Threading.Thread.CurrentThread.ManagedThreadId}]: ProcessForceStop() completed", DateTimeOffset.UtcNow);
        }
        ...
        {
            Console.WriteLine($"{{0:dd.MM.yyyy hh:mm.ss:ffff}}[{this}][{System.Threading.Thread.CurrentThread.ManagedThreadId}]: ProcessReap() [{socket}]", DateTimeOffset.UtcNow);
        ...
        {
            Console.WriteLine($"{{0:dd.MM.yyyy hh:mm.ss:ffff}}[{this}][{System.Threading.Thread.CurrentThread.ManagedThreadId}]: ProcessReaped() with {m_sockets} sockets", DateTimeOffset.UtcNow);
            --m_sockets;
            ...
            if (m_sockets == 0 && m_terminating)
            {
                ...
                m_poller.RemoveHandle(m_mailboxHandle);
                m_poller.Stop();
                Console.WriteLine($"{{0:dd.MM.yyyy hh:mm.ss:ffff}}[{this}][{System.Threading.Thread.CurrentThread.ManagedThreadId}]: ProcessReaped() with stop", DateTimeOffset.UtcNow);
            }

            Console.WriteLine($"{{0:dd.MM.yyyy hh:mm.ss:ffff}}[{this}][{System.Threading.Thread.CurrentThread.ManagedThreadId}]: ProcessReaped() completed.", DateTimeOffset.UtcNow);
        }
    }
}
