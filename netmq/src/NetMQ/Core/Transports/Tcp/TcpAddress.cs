...
#else
                Console.WriteLine($"{{0:dd.MM.yyyy hh:mm.ss:ffff}}: {addrStr} Start.", DateTimeOffset.UtcNow);
                var availableAddresses = Dns.GetHostEntry(addrStr).AddressList;
                Console.WriteLine($"{{0:dd.MM.yyyy hh:mm.ss:ffff}}: {addrStr} End.", DateTimeOffset.UtcNow);
#endif
