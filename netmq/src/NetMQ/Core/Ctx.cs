...
namespace NetMQ.Core
{
    ...
    internal sealed class Ctx
    {
        ...
        public SocketBase CreateSocket(ZmqSocketType type)
        {
            lock (m_slotSync)
            {
                Console.WriteLine("{0:dd.MM.yyyy hh:mm.ss:ffff}: CreateSocket() lock acquired", DateTimeOffset.UtcNow);
                if (m_starting)
                {
                    m_starting = false;
                ...
                Console.WriteLine($"{{0:dd.MM.yyyy hh:mm.ss:ffff}}[{s}][{System.Threading.Thread.CurrentThread.ManagedThreadId}]: created", DateTimeOffset.UtcNow);
