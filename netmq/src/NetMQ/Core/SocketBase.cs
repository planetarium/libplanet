...
namespace NetMQ.Core
{
    internal abstract class SocketBase : Own, IPollEvents, Pipe.IPipeEvents
    {
        ...
        {
            Console.WriteLine($"{{0:dd.MM.yyyy hh:mm.ss:ffff}}[{this}][{Thread.CurrentThread.ManagedThreadId}]: {addr} Bind() start", DateTimeOffset.UtcNow);
            Lock();
            try
            {
                CheckContextTerminated();
            ...
            {
                Console.WriteLine($"{{0:dd.MM.yyyy hh:mm.ss:ffff}}[{this}][{Thread.CurrentThread.ManagedThreadId}]: {addr} Bind() completed", DateTimeOffset.UtcNow);
                Unlock();
            }
        }
        ...
        {
            Console.WriteLine($"{{0:dd.MM.yyyy hh:mm.ss:ffff}}[{this}][{Thread.CurrentThread.ManagedThreadId}]: {addr} Connect() start", DateTimeOffset.UtcNow);
            Lock();
            try
            {
                CheckContextTerminated();
            ...
            {
                Unlock();
                Console.WriteLine($"{{0:dd.MM.yyyy hh:mm.ss:ffff}}[{this}][{Thread.CurrentThread.ManagedThreadId}]: {addr} Connect() finished", DateTimeOffset.UtcNow);
            }
        }
        ...
        internal void StartReaping(Poller poller)
        {
            ...
            // immediately.
            Terminate();
            Console.WriteLine($"{{0:dd.MM.yyyy hh:mm.ss:ffff}}[{this}][{Thread.CurrentThread.ManagedThreadId}]: CheckDestroy() start", DateTimeOffset.UtcNow);
            CheckDestroy();
            Console.WriteLine($"{{0:dd.MM.yyyy hh:mm.ss:ffff}}[{this}][{Thread.CurrentThread.ManagedThreadId}]: CheckDestroy() done", DateTimeOffset.UtcNow);
        }
        ...
        protected override void ProcessTerm(int linger)
        {
            ...
            Console.WriteLine($"{{0:dd.MM.yyyy hh:mm.ss:ffff}}[{this}][{Thread.CurrentThread.ManagedThreadId}]: ProcessTerm with {m_pipes.Count} pipes", DateTimeOffset.UtcNow);
            // Ask all attached pipes to terminate.
            for (int i = 0; i != m_pipes.Count; ++i)
                m_pipes[i].Terminate(false);
            Console.WriteLine($"{{0:dd.MM.yyyy hh:mm.ss:ffff}}[{this}][{Thread.CurrentThread.ManagedThreadId}]: PipeTerm done", DateTimeOffset.UtcNow);
            RegisterTermAcks(m_pipes.Count);
            Console.WriteLine($"{{0:dd.MM.yyyy hh:mm.ss:ffff}}[{this}][{Thread.CurrentThread.ManagedThreadId}]: TermAcks registered.", DateTimeOffset.UtcNow);

            // Continue the termination process immediately.
            base.ProcessTerm(linger);
            Console.WriteLine($"{{0:dd.MM.yyyy hh:mm.ss:ffff}}[{this}][{Thread.CurrentThread.ManagedThreadId}]: base.ProcessTerm with {linger} finished.", DateTimeOffset.UtcNow);
        }
        ...
        /// handlers explicitly. If required, it will deallocatre the socket.
        /// </summary>
