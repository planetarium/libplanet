    ...
    public static class NetMQConfig
    {
        ...
        public static void Cleanup(bool block = true)
        {
            lock (s_sync)
            {
                if (s_ctx != null)
                {
                    Console.WriteLine("{0:dd.MM.yyyy hh:mm.ss:ffff}: Cleanup called.", DateTimeOffset.UtcNow);
                    s_ctx.Terminate(block);
                    Console.WriteLine("{0:dd.MM.yyyy hh:mm.ss:ffff}: Terminated.", DateTimeOffset.UtcNow);
                    s_ctx = null;
                }
            }
        }
