...
### Added APIs
 ...
 -  Added `IConsensusCryptoBackend` interface, which contains VRF functionalities
    which is used on the consensus as a pseudo-random function.  [[#VRF]]
 -  Added `DefaultConsensusCryptoBackend` class as a default implementation of
    `IConsensusCryptoBackend`.  [[#VRF]]
 -  Added `CryptoConfig.ConsensusCryptoBackend` property as a VRF backend used on
    the consensus.  [[#VRF]]
